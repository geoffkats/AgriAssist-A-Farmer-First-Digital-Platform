  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Welcome to AgriAssist. Please log in to continue.',
      });
      router.push('/login');
    } catch (error) {
      toast({
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
       <Card className="w-full max-w-sm">
         <CardHeader className="text-center">
            ...
            <CardDescription>Join our network to source produce directly from farmers.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
           <div className="space-y-2">
            <Label htmlFor="name">Company / Full Name</Label>
            <Input id="name" placeholder="e.g., Kampala Agro Processors" required value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="email">Business Email</Label>
            <Input id="email" type="email" placeholder="procurement@yourcompany.com" required value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div className="space-y-2">
